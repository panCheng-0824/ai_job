...
def stream_chat_service_tokens(ctx: ChatStreamRunContext) -> ChatTokenStreamContext:
    ...
    双通道
    ----
    - ``resume_advise``：段落优化建议（原文 vs 改后）；
    - ``resume_generate``：综合素材生成结构化简历，经 ``resume_render`` 事件供前端渲染。
    """
    ...
    # 简历页「提交优化」约定 use_role_pipeline=false：单轮任务，不拼接历史会话摘录。
    ...
    llm_timeout = resolve_role004_llm_timeout_seconds(merged_user)
