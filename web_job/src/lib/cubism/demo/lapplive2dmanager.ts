...
export class LAppLive2DManager {
  ...
  public changeModel(modelName: string): void {
  ...
  public isModelReady(): boolean {
    return Boolean(this._models[0]?.getModel());
  }

  public applyPresenceState(
    ...
    switch (state) {
      ...
      case "listening":
        this.applyExpression(model, "listening");
        break;
      case "thinking":
        this.applyExpression(model, "thinking");
        break;
      case "idle":
      default:
        this.applyExpression(model, "idle");
        model.startRandomMotion(
          LAppDefine.MotionGroupIdle,
  ...
  private applyExpression(
    model: LAppModel,
    kind: "idle" | "listening" | "thinking"
  ): void {
    const map = LAppDefine.ModelExpressions[this._currentModelName];
    const expressionId = map?.[kind];
    if (expressionId) {
  ...
  public playExpression(expressionId: string): void {
    const model = this._models[0];
    if (!model?.getModel() || !expressionId) return;
    model.setExpression(expressionId);
  }

  public playMotion(group: string, index: number): void {
    const model = this._models[0];
    if (!model?.getModel() || !group) return;
    model.startMotion(group, index, LAppDefine.PriorityNormal);
  }

  public playHitInteraction(areaName: string): void {
    const model = this._models[0];
    if (!model?.getModel()) return;
    if (areaName === LAppDefine.HitAreaNameHead) {
      model.setRandomExpression();
      return;
    }
    if (areaName === LAppDefine.HitAreaNameBody) {
      const handle = model.startRandomMotion(
        LAppDefine.MotionGroupTapBody,
        LAppDefine.PriorityNormal
      );
  ...
  private _currentModelName = LAppDefine.ModelDir[0];
  private _lastPresenceState: string | null = null;
